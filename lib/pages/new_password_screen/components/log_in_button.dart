...
class NewPasswordScreenLoginButton extends StatelessWidget {
  const NewPasswordScreenLoginButton({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return context.watch<StateNewPasswordScreen>().logInState?
    Padding(
      padding: EdgeInsets.only(top: 41.0 * DeviceInfo.h),
      child: Material(
        color: Colors.transparent,
        child: InkWell(
          overlayColor: MaterialStateProperty.all(Colors.transparent),
          onTap: () {
            context.read<StateNewPasswordScreen>().onPressedLogIn(context);
          },
          child: DecoratedBox(
